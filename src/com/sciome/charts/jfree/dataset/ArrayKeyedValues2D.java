...
public class ArrayKeyedValues2D implements Serializable {
	 /** For serialization. */
     ...
     */
    public ArrayKeyedValues2D(boolean sortRowKeys) {
     ...
     */
    public ArrayList<Number> getValue(int row, int column) {
        ArrayList<Number> result = null;
        ArrayKeyedValues rowData = (ArrayKeyedValues) this.rows.get(row);
        if (rowData != null) {
     ...
     */
    public ArrayList<Number> getValue(Comparable rowKey, Comparable columnKey) {
        ...
        if (row >= 0) {
            ArrayKeyedValues rowData
                = (ArrayKeyedValues) this.rows.get(row);
     ...
     * @see #removeValue(Comparable, Comparable)
     */
    public void addValue(ArrayList<Number> value, Comparable rowKey,
     ...
     * @see #removeValue(Comparable, Comparable)
     */
    public void setValue(ArrayList<Number> value, Comparable rowKey,
                         Comparable columnKey) {

        ArrayKeyedValues row;
        int rowIndex = getRowIndex(rowKey);

        if (rowIndex >= 0) {
            row = (ArrayKeyedValues) this.rows.get(rowIndex);
        }
        else {
            row = new ArrayKeyedValues();
            if (this.sortRowKeys) {
                rowIndex = -rowIndex - 1;
