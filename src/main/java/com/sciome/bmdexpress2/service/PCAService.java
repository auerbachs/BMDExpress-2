...
public class PCAService implements IPCAService
{
	...
	public PCAResults calculatePCA(DoseResponseExperiment doseResponseExperiment)
	{
		...
		PCA pca = new PCA(MatrixUtils.createRealMatrix(numericMatrix).transpose(), CovarianceType.COVARIANCE,
				4);
		...
		for (int i = 0; i < transformedData.getRowDimension(); i++)
		{
			...
			singleRow.setDosage(
					doseVector[i] + " - " + doseResponseExperiment.getTreatments().get(i).getName());
